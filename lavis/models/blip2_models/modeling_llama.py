...
class LlamaModelTextualInversion(LlamaPreTrainedModel):
    # NOTE: This is the self-defined class for adding pseudo-word embedding in LLM.
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx, dtype=torch.float32)
        self.layers = nn.ModuleList([LlamaDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        
        # NOTE: This is the pseudo-word embedding
        self.train_embed_tokens = nn.Embedding(1, config.hidden_size, dtype=torch.float32)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            # inputs_embeds = self.embed_tokens(input_ids) # Original method
            ...
            inputs_embeds = self.embed_tokens(pretrained_input_ids)
            
            input_ids = input_ids - vocab_size
            input_ids[~train_word_mask] = 0
            train_inputs_embeds = self.train_embed_tokens(input_ids)
...
class LlamaForCausalLMTextualInversion(LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        #self.model = LlamaModel(config)
        self.model = LlamaModelTextualInversion(config)
